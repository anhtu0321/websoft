...
$sql = "select * from baocaongay where id='$id'";
...
<div class = "col-sm-12 chi-tiet-tin">
    ...
    <div class="col-sm-12 no-paddingn text-right">
        <a href="bcnuploads/<?php echo $rs['file'];?>"><span class="glyphicon glyphicon-download-alt"></span> File đính kèm</a>
        <hr/>
    </div>
    <!-- Tin cùng chuyên mục -->
<?php
// lay id lon nhat
    $sqlmax = "select id from baocaongay where id> $id order by id DESC limit 5";
    $tbmax = mysqli_query($con, $sqlmax); 
    $rsmax = mysqli_fetch_array($tbmax);
    $max = $rsmax["id"];
    if ($max == ""){$max = $id;}
// lay id nho nhat
    $sqlmin = "select id from baocaongay where id < $id order by id ASC limit 5";
    $tbmin = mysqli_query($con, $sqlmin); 
    $rsmin = mysqli_fetch_array($tbmin);
    $min = $rsmin["id"];
    if ($min == ""){$min = $id;}
$sqlcungmuc = "select id, tieude, ngaynhap from baocaongay where id <> '$id' and id BETWEEN $min and $max order by id DESC limit 10";
...
?>
    <div class="col-sm-12 no-padding tin-chuyen-muc">
        ...
        <ul>
            ...
            ?>
                    <li><a href="index.php?view=chitietbcn&id=<?php echo $rscungmuc['id'];?>"><?php echo $rscungmuc["tieude"];?></a><br></li>
            <?php
                }
            ?>
        </ul>
